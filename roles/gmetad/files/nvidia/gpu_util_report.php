...
function graph_gpu_util_report ( &$rrdtool_graph ) {
    ...
    if (!file_exists("${rrd_dir}/gpu0_util.rrd")) {
       return;
    }
    ...
    $gpu_count = exec("find ${rrd_dir}/gpu?_util.rrd | wc -l");
    $series = '';
    foreach (range(0, $gpu_count-1) as $i) {
       $series .= "DEF:'gpu$i'='${rrd_dir}/gpu${i}_util.rrd':'sum':AVERAGE ";
    }
    foreach (range(0, $gpu_count-1) as $i) {
       if ($i == 0) {
